...
def execute(commands, input=None):
    "Execute commands in a system-independent manner, return 3 objects: output, error and return code"
...
def run_hmmsearch(query_hmmfile, fasta_file):
    "Run hmmsearch"
	#ad-hoc: specification of the path of the HMMER source code 
    command = ["hmmsearch","--domT", "50", "--domtblout", "temp.out", "-o", "temp.fasta", query_hmmfile, fasta_file]
    try:
        out, err, retcode = execute(command)
	out = open("temp.out","r").read()
    except OSError:
        return []
	print "OSEerror within the run_hmmsearch function"
...
def read_fasta_file(fastafile):
    ...
    for entry in entries:
        ...
        #print accession #THAT FUNCTION WORKS
...
def run_HMMer(hmmfile, fastafile):
	...
	#print hsp #print the header of the of the hmmsearch output file
...
def write_fasta(fastadict, outfile, ks_hmmer_results, at_hmmer_results):
    ...
    domnr = 1
    for cds in ks_hmmer_results.keys():#for each KS domain hit
        #Get sequence of the hit from the initial fastadict
	    ...
	    #if an AT domain is present near the KS domain, it means that it is not a trans-AT PKS
            transAT = True
            if at_hmmer_results.has_key(cds):
                ...
                for at_hit in at_hits:
                    if at_hit.hit_start > hit.hit_end and int(at_hit.hit_start) - int(hit.hit_end) < 400:
                        transAT = False
			#print "KS domain part of a cis-AT PKS, not written in the ouput file" 
...
if __name__ == "__main__":
    ...
    ks_hmmfile = "antismash/generic_modules/hmm_detection/PKS_KS.hmm"
    at_hmmfile = "antismash/generic_modules/hmm_detection/PKS_AT.hmm"
    fastafile = "subset_aligned.afa"
    outfile = "transKS_subset.fasta"
    ...
    print ks_hmmer_results
    print at_hmmer_results
